...
\newpage 
\section{Lecture 18: Hausdorff-Young Theorem for Fourier Transform}
\subsubsection{Key Topics:} (1) Hausdorff-Young Theorem for Fourier Transform, (2) Fourier Series, (3) Harmonic Polynomials in $L^{2}(S^{n-1})$
\subsubsection{Summary:} In this lecture, we will continue with our discussion of the Fourier Transform (in particular, we will examine ways to extend the transform to $L^{2}(\mathbb{R}^{n})$ from $L^{1}(\mathbb{R}^{n})$). In addition, we will examine various properties of and results for the Fourier Series in higher dimensions. 

\subsubsection{Lecture:}

\begin{itemize}
	\item \textbf{Cor: (Corollary of Plancherel's Identity)} The Fourier transform can be extended to $L^{2}(\mathbb{R}^{n})$, and the Fourier transform is an isometry on $L^{2}(\mathbb{R}^{n})$. Moreover, for $f \in L^{2}(\mathbb{R}^{n})$, 
		\begin{equation}
			\int_{[-A,A]^{n}}f(x)e^{-2\pi i (\xi, x)}\;dx \longrightarrow \hat{f}(\xi) \in L^{2}(\mathbb{R}^{n}) \qquad \text{ as $A \to \infty$}.
		\end{equation}
	
	{\color{orange}
		Fix $f \in L^{2}(\mathbb{R}^{n})$. By the density of $L^{1} \cap L^{2}(\mathbb{R}^{n})$ in $L^{2}(\mathbb{R}^{n})$, there exists a sequence $\{f_{j}\}_{j = 1}^{\infty} \subset L^{1} \cap L^{2}(\mathbb{R}^{n})$ that converges to $f$ under the $L^{2}$-norm. This means that $\{f_{j}\}$ is a Cauchy sequence in $L^{2}(\mathbb{R}^{n})$. By Plancherel's identity, since $\norm{f_{j}}_{2} = \norm{\hat{f}_{j}}_{2}$, it follows that $\{\hat{f}_{j}\}$ is also a Cauchy sequence in $L^{2}(\mathbb{R}^{n})$, and hence converges to some $\hat{f} \in L^{2}(\mathbb{R}^{n})$. We shall define this limit $\hat{f}$ as the Fourier transform of $f \in L^{2}(\mathbb{R}^{n})$, so that the Fourier transform can be extended to the space of square-integrable functions. Then since the $L^{2}$ norm is continuous, one has, 
			\begin{equation}
				\norm{\hat{f}}_{2} = \norm{\lim_{j \to \infty}\hat{f}_{j}}_{2} = \lim_{j \to \infty}\norm{\hat{f}_{j}}_{2} = \lim_{j \to \infty}\norm{f_{j}}_{2} = \norm{f}_{2},
			\end{equation}
		so that the Fourier transform is an isometry of spaces. Now we shall construct an explicit formula for $\hat{f}(\xi)$. For each positive integer $j > 0$, define $f_{j} = f \cdot \chi_{[-j,j]^{n}}$. Then each $f_{j} \in L^{1} \cap L^{2}(\mathbb{R}^{n})$ and 
			\begin{align}
				\begin{split}
					\norm{f_{j} - f}_{2}^{2} &= \int_{\mathbb{R}^{n}}|f \cdot (\chi_{[-j,j]^{n}} -1)|^{2}\;dx 
				\end{split}
			\end{align}
		Pointwise, $|f \cdot (\chi_{[-j,j]^{n}} - 1)|^{2} \to 0$ and 
			\begin{equation}
				|f \cdot (\chi_{[-j,j]^{n}} - 1)|^{2} \leq |f|^{2} \in L^{1}(\mathbb{R}^{n}), 
			\end{equation}
		so that by the Dominated Convergence Theorem, 
			\begin{equation}
				\norm{f_{j} - f}_{2}^{2} = \int_{\mathbb{R}^{n}}|f \cdot (\chi_{[-j,j]^{n}} -1)|^{2}\;dx  \to 0. 
			\end{equation}
		Hence, the $f_{j} \to f$ under the $L^{2}$-norm. Therefore, by the previous argument, $\hat{f}_{j} \to \hat{f}$ under the $L^{2}$-norm. More precisely, 
			\begin{equation}
				\int_{[-j,j]^{n}}f(x)e^{-2\pi i(\xi, x)}\;dx = \hat{f}_{j} \longrightarrow \hat{f} \qquad \text{ as $j \to \infty$}.
			\end{equation}
	}
	
	\item \textbf{Thm. (Riesz-Thorin Interpolation Theorem)} Let $1 \leq p_{0}, q_{0} \leq \infty$, $1 \leq p_{1}, q_{1} \leq \infty$. Assume that $(X, \mathscr{M}, \mu)$, $(Y, \mathscr{N}, \nu)$ are two measure spaces. Let $T: L^{p_{0}}(X)+ L^{p_{1}}(X)\to L^{q_{0}}(Y) + L^{q_{1}}(Y)$ be a linear map such hat 
		\begin{equation}
			T: L^{p_{0}}(X) \to L^{q_{0}}(Y), L^{p_{1}}(X) \to L^{q_{1}}(Y)
		\end{equation}
	are bounded linear maps with $\norm{T}_{p_{0} \to q_{0}} \leq M_{0}$ and $\norm{T}_{p_{1} \to q_{1}} \leq M_{1}$. Then $T: L^{p_{0}}(X) \to L^{q_{0}}(Y)$ is also bounded linear with 
		\begin{equation}
			\norm{T}_{p_{0} \to q_{0}} \leq M_{0}^{1 - \theta}M_{1}^{\theta},
		\end{equation}
	where 
		\begin{equation}
			\left(\frac{1}{p_{\theta}}, \frac{1}{q_{\theta}}\right) = (1 - \theta)\left(\frac{1}{p_{0}}, \frac{1}{q_{0}}\right) + \theta\left(\frac{1}{p_{1}}, \frac{1}{q_{1}}\right). 
		\end{equation}
		
	\item \textbf{Thm. (Hausdorff-Young)} Assume that $1 \leq p \leq 2$, and $f \in L^{1} \cap L^{p}(\mathbb{R}^{n})$. Then $\hat{f} \in L^{q}(\mathbb{R}^{n})$, with 
		\begin{equation}
			\norm{\hat{f}}_{q} \leq \norm{f}_{p}, \qquad \text{ where $p^{-1} + q^{-1} = 1$}.
		\end{equation}
		
	\item \textbf{Cor. (Extending the Fourier Transform to $L^{p}$-Spaces)} The Fourier Transform is a bounded linear map from $L^{p}(\mathbb{R}^{n})$ to $L^{q}(\mathbb{R}^{n})$, for $1 \leq p < 2$ and $p^{-1} + q^{-1} = 1$, but is not an isometry. 
	
	{\color{orange}
		Fix $1 \leq p < 2$, and $f \in L^{p}(\mathbb{R}^{n})$. Since $L^{1} \cap L^{p}(\mathbb{R}^{n})$ is dense in $L^{p}(\mathbb{R}^{n})$, there exists a sequence $\{f_{j}\}_{j= 1}^{\infty} \subset L^{1} \cap L^{p}(\mathbb{R}^{n})$ that converges to $f$ under the $L^{p}$-norm. Since each $f_{j} \in L^{1}(\mathbb{R}^{n})$, their Fourier transforms $\hat{f}_{j}$ are well-defined. Moreover, by the Hausdorff-Young Inequality,
		\begin{equation}
			\norm{\hat{f}_{j} - \hat{f}_{\ell}}_{q} = \norm{\wh{f_{j} - f_{\ell}}}_{q} \leq \norm{f_{j} - f_{\ell}}_{p} \longrightarrow 0 \text{ as $j, \ell \to \infty$}. 
		\end{equation}
		Hence, $\{\hat{f}_{j}\}_{j = 1}^{\infty}$ is a Cauchy sequence in $L^{q}(\mathbb{R}^{n})$; since this space is complete under the $L^{q}$-norm, we conclude that $\hat{f}_{j}$ converges in the space. We define the Fourier transform of $f$ to be $L^{q}(\mathbb{R}^{n}) \ni \hat{f} \coloneqq \lim_{j \to\infty}\hat{f}_{j}$. However, this time, we have 
			\begin{equation}
				\norm{\hat{f}}_{q} = \lim_{j \to \infty}\norm{\hat{f}_{j}}_{q} \leq \lim_{j \to \infty}\norm{f_{j}}_{p} = \norm{f}_{p}, 
			\end{equation}
		so that the Fourier transform is \textit{not} an isometry. However, for each positive integer $j > 0$, define $f_{j} = f \cdot \chi_{[-j,j]^{n}}$. Then each $f_{j} \in L^{1} \cap L^{p}(\mathbb{R}^{n})$ and 
		\begin{align}
			\begin{split}
				\norm{f_{j} - f}_{p}^{p} &= \int_{\mathbb{R}^{n}}|f \cdot (\chi_{[-j,j]^{n}} -1)|^{p}\;dx 
			\end{split}
		\end{align}
		Pointwise, $|f \cdot (\chi_{[-j,j]^{n}} - 1)|^{p} \to 0$ and 
		\begin{equation}
			|f \cdot (\chi_{[-j,j]^{n}} - 1)|^{p} \leq |f|^{p} \in L^{1}(\mathbb{R}^{n}), 
		\end{equation}
		so that by the Dominated Convergence Theorem, 
		\begin{equation}
			\norm{f_{j} - f}_{p}^{p} = \int_{\mathbb{R}^{n}}|f \cdot (\chi_{[-j,j]^{n}} -1)|^{p}\;dx  \to 0. 
		\end{equation}
		Hence, the $f_{j} \to f$ under the $L^{p}$-norm. Therefore, by the previous argument, $\hat{f}_{j} \to \hat{f}$ under the $L^{q}$-norm. More precisely, 
		\begin{equation}
			\int_{[-j,j]^{n}}f(x)e^{-2\pi i(\xi, x)}\;dx = \hat{f}_{j} \longrightarrow \hat{f} \qquad \text{ as $j \to \infty$}.
	\		\end{equation}
	}
	...
	\item \textbf{Prop. (Orthonormal Basis for $L^{2}(S^{1}))$} Identifying $[0,1]$ with $S^{1}$ by means of the identification $0 \sim 1$, the collection $\{e^{2\pi i k x}\}_{k \in \mathbb{Z}}$ forms an orthonormal basis of $L^{2}(S^{1})$. 
	
	{\color{orange}
		If $k \neq \ell$, we compute, 
			\begin{align}
				\begin{split}
					\braket{e^{2\pi i k x}, e^{2\pi i \ell x}} &= \int_{0}^{1}e^{2\pi i (k - \ell)x}\;dx \\
					&= \frac{1}{2\pi i (k - \ell)}e^{2\pi i (k - \ell)x} \bigg|_{0}^{1} \\
					&= \frac{1}{2\pi i (k- \ell)}\cdot \left(e^{2\pi i (k - \ell)} - 1\right). 
				\end{split}
			\end{align}	
		Since $k, \ell$ are integers, $k - \ell$ is an integer. And therefore, $e^{2\pi i (k - \ell)} = 1$ so that $\braket{e^{2\pi i k x}, e^{2\pi i k\ell}} = 0$. On the other hand, if $k = \ell$, then 
			\begin{equation}
				\braket{e^{2\pi i k x}, e^{2\pi i \ell x}} = \int_{0}^{1}e^{2\pi i (k - \ell)x}\;dx = \int_{0}^{1}\;dx = 1. 
			\end{equation}
		This concludes the proof. 
	}
	
	\item \textbf{Prop. (Basic Properties of $L^{2}(S^{n-1})$)} We have the following facts: \vspace{-0.25cm}
		\begin{enumerate}[itemsep =-2pt,label = (\arabic{*})]
			\item Let $\mathcal{H}_{k}$ be the space of all homogeneous harmonic polynomials with degree $k$ in $\mathbb{R}^{n}$. Then $\mathcal{H}_{k} \perp \mathcal{H}_{\ell}$, $k \neq \ell$ under $L^{2}(S^{n-1})$. 
			\item $L^{2}(S^{n-1}) = \bigoplus_{k =0}^{\infty}\mathcal{H}_{k}$. 
			\item $\mathcal{H}_{k}$ is the eigenspace of $\Delta_{S^{n}}$ with eigenvalue $-k(k  +n -2)$.
		\end{enumerate}
\end{itemize}

\newpage 
\section{Lectures 19 - 20: Definitions in Distribution Theory}
\subsubsection{Key Topics:} (1) Distributions, (2) Weak Solutions 
\subsubsection{Summary:}
In this lecture, we will introduce the idea of distributions, motivations for working with distributions, and some important operations on distributions. These two lectures are merged together for continuity. 
\subsubsection{Lecture:}
\begin{itemize}
	...
	\item \textbf{Prop. (Fundamental Result in Distribution Theory)} For any domain $U \subseteq \mathbb{R}^{n}$, $C_{c}^{\infty}(U)$ is dense in $\mathcal{O}'(U)$ in the topology of $\mathcal{O}'(U)$. More precisely, for every $\ell \in \mathcal{O}'(U)$, there exists $\{f_{j}\} \subset C_{c}^{\infty}(U)$ so that for all $\phi \in \mathcal{O}(U)$, 
		\begin{equation}
			\ell(\phi) = \int_{U}\phi f_{j}\;dx. 
		\end{equation}
	\end{itemize}
