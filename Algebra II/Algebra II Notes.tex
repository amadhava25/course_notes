...
\section{Basic Category Theory}
	...
	\begin{remark} Because of the above result, the functors $\yo^\bullet$ and $\yo_\bullet$ are often called the ``\textbf{(resp. contravariant, covariant) Yoneda \textit{embedding}}", as we see that these functors are fully faithful (and isomorphism reflecting).\footnote{Sometimes, it is considered useful to view certain nice contravariant functors from certain nice (``geometric/topological") categories as ``generalized spaces" (``sheaves and/or algebraic spaces") which arise from gluing the objects/spaces in the base category. In this view, a ``sheaf" which is isomorphic to $\yo^c$ is said to be ``represented by $c$"-- such matters are quite relevant to the study of moduli spaces/moduli problems. }
		\end{remark}
	Philosophically, here's the point: ``objects in $\mathcal{C}$ are determined by morphisms to (and/or from) them". This is, more or less, a far \textit{far} reaching generalization of Cayley's theorem in group theory (``every group is a symmetry group of some object").
