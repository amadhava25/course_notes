...
\section{Tensor products (even over potentially non-commutative rings) and more miscellaneous module theory}
	...
	\begin{definition}A $\delta$-functor, $H^*$ \textbf{universal} if
		\end{definition}
	\begin{theorem}\label{FHHF}(FOAG 1.5.I, ``the FHHF theorem") Suppose $F: \mathcal{A}\to \mathcal{B}$ is a functor of abelian categories, and $C^\bullet$ an $\mathcal{A}$-complex.
		\begin{itemize}
			...
			\item If $F$ is exact, then there is a natural isomorphism $FH ^\bullet \iso H^\bullet F$.
		\end{itemize}
		\end{theorem}
	\begin{proof}
